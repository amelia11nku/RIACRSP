...
class HybridLearning:
    ...
    def optimize(self):
        ...
        logger.info(f"Starting optimization, total iterations: {self.config.total_max_iterations}")
        logger.info(f"Minimum iterations for pure SL: {self.config.pure_sl_min_iterations}")
        logger.info(f"Stagnation detection window size: {self.config.stagnation_window_size}")
        logger.info(f"Stagnation threshold: {self.config.stagnation_threshold}")
        logger.info(f"Force switch iterations: {self.config.force_switch_iterations}")
        ...
        while iteration < self.config.total_max_iterations:
            ...
            logger.info(f"Iteration {iteration+1}/{self.config.total_max_iterations}: Phase={algorithm_phase}, SL ratio={sl_ratio:.2f}, EL ratio={el_ratio:.2f}")
