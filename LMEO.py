...
class LMEO:
    ...
    def machine_local_search1(self, child_machine: List[int], child_agv_w: List[int], 
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """机器分配局部搜索1
        
        先找到完工时间最长的作业，然后从该作业中随机选取一个工序，将该工序所分配的机器
        随机替换成另一个可用机器。
        ...
        # 第1步: 获取工序列表和各作业的工序信息
        job_operations = self._get_job_operations(self.operations_list)
        
        # 第2步: 模拟当前调度方案，获取各作业的完工时间
        _, schedule_results = calculate.simulate(child_machine, child_agv_w, child_agv_f, child_schedule, return_schedule=True)
        if schedule_results is None:
            return child_machine, current_makespan
        
        # 第3步: 计算每个作业的完工时间
        job_completion_times = self._calculate_job_completion_times(schedule_results, job_operations)
        if not job_completion_times:
            return child_machine, current_makespan
            
        # 第4步: 找到完工时间最长的作业及其工序
        longest_job_id = max(job_completion_times, key=job_completion_times.get)
        job_ops = job_operations.get(longest_job_id, [])
        if not job_ops:
            return child_machine, current_makespan
            
        # 第5步: 选择一个工序并替换其机器
        selected_op, op_idx = self._select_operation_with_multiple_machines(job_ops, self.operations_list, child_machine)
        if selected_op is None:
            return child_machine, current_makespan
            
        # 第6步: 替换机器并计算新的完工时间
        new_machine = child_machine.copy()
        new_machine[op_idx] = selected_op['new_machine']
    ...
    def _get_job_operations(self, operation_keys: List[str]) -> Dict[int, List[str]]:
        """按作业组织工序
        
        Args:
            operation_keys: 工序ID列表
            
        Returns:
            Dict[int, List[str]]: 按作业ID组织的工序列表
        """
        job_operations = {}
        
        for op_id in operation_keys:
            parts = op_id.split('_')
            if len(parts) >= 2:
                job_id = int(parts[0][1:])  # 提取作业ID，去掉 'o' 前缀
                
                if job_id not in job_operations:
                    job_operations[job_id] = []
                    
                job_operations[job_id].append(op_id)
                
        return job_operations
    
    def _calculate_job_completion_times(self, schedule_results: List[Dict], job_operations: Dict[int, List[str]]) -> Dict[int, float]:
        """计算每个作业的完工时间
        
        Args:
            schedule_results: 调度结果
            job_operations: 按作业ID组织的工序列表
            
        Returns:
            Dict[int, float]: 每个作业的完工时间
        """
        # 初始化各作业的开始和结束时间
        job_times = {job_id: {'start': float('inf'), 'end': 0} for job_id in job_operations}
        
        # 从调度结果中提取机器处理数据
        for item in schedule_results:
            if 'device' in item and item['device'].startswith('M') and 'operation' in item:
                op_id = item['operation']
                parts = op_id.split('_')
                ...
                if len(parts) >= 2:
                    ...
                    if job_id in job_times:
                        ...
                        job_times[job_id]['start'] = min(job_times[job_id]['start'], start_time)
                        job_times[job_id]['end'] = max(job_times[job_id]['end'], end_time)
        
        # 计算每个作业的完工时间
        job_completion_times = {}
        for job_id, times in job_times.items():
            if times['start'] < float('inf') and times['end'] > 0:
                job_completion_times[job_id] = times['end'] - times['start']
                
        return job_completion_times
    
    def _select_operation_with_multiple_machines(self, job_ops: List[str], operation_keys: List[str], child_machine: List[int]) -> Tuple[Optional[Dict], Optional[int]]:
        """选择一个有多个可用机器的工序
        
        Args:
            job_ops: 作业的工序列表
            operation_keys: 全局工序ID列表
            child_machine: 机器分配序列
            
        Returns:
            Tuple[Optional[Dict], Optional[int]]: 选中的工序信息和索引
        """
        # 随机打乱工序列表，以便随机选择
        random.shuffle(job_ops)
        
        for op_id in job_ops:
            try:
                op_idx = operation_keys.index(op_id)
            except ValueError:
                continue
    ...
    def machine_local_search2(self, child_machine: List[int], child_agv_w: List[int], 
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """机器分配局部搜索2
        
        先选择分配工序数量最多的机器，再从该机器所分配的工序中随机选择一个工序
        并将该工序分配到另一个可用机器，若没有其他可用机器，则随机选择另一个工序。
        ...
        # 第1步: 统计每个机器分配的工序数量
        machine_operations = {}
        for op_idx, machine_id in enumerate(child_machine):
            if machine_id not in machine_operations:
                machine_operations[machine_id] = []
            machine_operations[machine_id].append(op_idx)
        
        # 第2步: 找到分配工序数量最多的机器
        if not machine_operations:
            return child_machine, current_makespan
            
        busiest_machine = max(machine_operations, key=lambda m: len(machine_operations[m]))
        busiest_ops = machine_operations[busiest_machine]
        
        # 第3步: 随机打乱该机器的工序列表，以便随机选择
        random.shuffle(busiest_ops)
        
        # 第4步: 尝试为每个工序找到替代机器
        for op_idx in busiest_ops:
            ...
            # 找到可替换的机器（排除当前机器）
            alternative_machines = [int(m) for m in available_machines if int(m) != busiest_machine]
            
            # 如果有可替换的机器，随机选择一个并替换
            if alternative_machines:
                new_machine = child_machine.copy()
                new_machine[op_idx] = random.choice(alternative_machines)
    ...
    def agv_local_search1(self, child_machine: List[int], child_agv_w: List[int],
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """AGV分配局部搜索1
        
        先选择完工时间最长的AGV_W，再从该AGV所分配的工序中随机选择一个工序
        并将该工序分配到另一个AGV_W，计算新的完工时间。
        ...
        # 第1步: 获取工序ID列表并模拟当前调度方案
        _, schedule_results = calculate.simulate(child_machine, child_agv_w, child_agv_f, child_schedule, return_schedule=True)
        
        if schedule_results is None:
            return child_agv_w, current_makespan
        
        # 第2步: 统计每个AGV_W的工作时间和分配的工序
        agv_w_times = {}  # 记录每个AGV_W的工作时间
        agv_w_operations = {}  # 记录每个AGV_W分配的工序索引
        
        # 初始化数据结构
        for agv_id in range(1, self.num_agvs_w + 1):
            agv_w_times[agv_id] = 0
            agv_w_operations[agv_id] = []
        
        # 从调度结果中提取AGV_W的工作时间
        for item in schedule_results:
            if 'device' in item and item['device'].startswith('AGV_W'):
                agv_id = int(item['device'].split('_')[-1])  # 提取AGV编号
                work_time = float(item['end_time']) - float(item['start_time'])
                agv_w_times[agv_id] += work_time
                
                # 找到对应的工序索引
                if 'operation' in item:
                    op_id = item['operation']
                    try:
                        op_idx = self.operations_list.index(op_id)
                        agv_w_operations[agv_id].append(op_idx)
                    except ValueError:
                        pass
        ...
        busiest_agv = max(agv_w_times, key=agv_w_times.get)
        busiest_ops = agv_w_operations[busiest_agv]
        
        # 如果该AGV没有分配工序，返回原方案
        if not busiest_ops:
            return child_agv_w, current_makespan
        
        # 第4步: 随机选择一个工序并将其分配到另一个AGV_W
        # 随机打乱工序列表，以便随机选择
        random.shuffle(busiest_ops)
        op_idx = busiest_ops[0]  # 选择第一个工序
        
        # 随机选择一个不同的AGV_W
        alternative_agvs = [agv_id for agv_id in range(1, self.num_agvs_w + 1) if agv_id != busiest_agv]
        if not alternative_agvs:
            return child_agv_w, current_makespan
        new_agv = random.choice(alternative_agvs)
        
        # 第5步: 替换AGV_W并计算新的完工时间
        new_agv_w = child_agv_w.copy()
        new_agv_w[op_idx] = new_agv
    ...
    def agv_local_search2(self, child_machine: List[int], child_agv_w: List[int],
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """AGV分配局部搜索2
    ...
    def agv_local_search3(self, child_machine: List[int], child_agv_w: List[int],
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """AGV分配局部搜索3
        
        随机选择一个工序，然后将该工序所分配的AGV替换为另一个AGV。
        ...
        # 第1步: 随机选择一个工序
        op_idx = random.randint(0, num_operations - 1)
        
        # 第2步: 获取当前分配的AGV_W
        current_agv_w = child_agv_w[op_idx]
        
        # 第3步: 随机选择一个不同的AGV_W
        alternative_agvs = [agv_id for agv_id in range(1, self.num_agvs_w + 1) if agv_id != current_agv_w]
        
        # 如果没有其他可用的AGV_W，尝试随机选择另一个工序
        if not alternative_agvs:
            # 如果只有一个AGV_W，直接返回
            ...
            # 随机选择另一个工序
            new_op_idx = (op_idx + random.randint(1, num_operations - 1)) % num_operations
            op_idx = new_op_idx
            current_agv_w = child_agv_w[op_idx]
            alternative_agvs = [agv_id for agv_id in range(1, self.num_agvs_w + 1) if agv_id != current_agv_w]
            
            # 如果仍然没有其他可用的AGV_W，直接返回
            if not alternative_agvs:
                return child_agv_w, current_makespan
        
        # 第4步: 随机选择一个新的AGV_W
        new_agv_w = random.choice(alternative_agvs)
        
        # 第5步: 创建新的AGV_W分配序列
        new_agv_w_list = child_agv_w.copy()
        new_agv_w_list[op_idx] = new_agv_w
        
        # 第6步: 计算新的完工时间
        new_makespan = calculate.simulate(child_machine, new_agv_w_list, child_agv_f, child_schedule)
        
        # 如果新的完工时间更短，则返回新的AGV_W分配序列
        if new_makespan is not None and new_makespan < current_makespan:
            return new_agv_w_list, new_makespan
        
        return child_agv_w, current_makespan

    def schedule_local_search1(self, child_machine: List[int], child_agv_w: List[int],
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """schedule排序局部搜索1
        
        基于NEH算法的思想, 从第2个元素开始, 依次尝试将每个元素插入到其之前的所有位置,
        选择使完工时间最小的插入位置。
        ...
        Returns:
            Tuple[List[int], float]: 改进后的schedule序列和完工时间
        """
        
        # 复制原始调度序列，以便进行修改
        best_schedule = child_schedule.copy()
        best_makespan = current_makespan
        # 创建工序索引到工序ID的映射
        op_index_to_id = {i+1: op_id for i, op_id in enumerate(self.operations_list)}

        # 从第2个元素开始逐个试探
        for i in range(1, len(child_schedule)):
            # 当前要试探的元素
            current_element = best_schedule[i]
            
            # 保存当前最优的插入方案
            best_position = i  # 默认保持原位置
            temp_best_makespan = best_makespan
            ...
            for j in range(0, i + 1):
                # 创建新的调度序列，将当前元素插入到位置j
                new_schedule = temp_schedule.copy()
                new_schedule.insert(j, current_element)
                
                # 检查插入后的序列是否合法
                try:
                    self.calculate._validate_schedule_string(new_schedule, op_index_to_id, priority_dict)
                except ValueError:
                    continue  # 如果不合法，跳过这个位置
                
                # 如果合法，则计算新序列的完工时间
                new_makespan = self.calculate.simulate(child_machine, child_agv_w, child_agv_f, new_schedule)
                
                # 如果新的完工时间更小，更新最优方案
                if new_makespan is not None and new_makespan < temp_best_makespan:
                    temp_best_makespan = new_makespan
                    best_position = j
            
            # 如果找到了更好的插入位置
            if best_position != i:
                # 创建新的最优调度序列
                new_best_schedule = best_schedule.copy()
                current_element = new_best_schedule.pop(i)
                new_best_schedule.insert(best_position, current_element)
                
                # 更新最优调度序列和完工时间
                best_schedule = new_best_schedule
                best_makespan = temp_best_makespan
        
        # 如果找到了更好的调度序列
        if best_makespan < current_makespan:
            return best_schedule, best_makespan
        
        return child_schedule, current_makespan

    def schedule_local_search2(self, child_machine: List[int], child_agv_w: List[int],
                        child_agv_f: List[int], child_schedule: List[int], current_makespan: float) -> Tuple[List[int], float]:
        """schedule排序局部搜索2
        
        随机交换两个不同元素的位置，并计算新的完工时间。
        ...
        for _ in range(4):
            # 随机选择两个不同的位置
            pos1 = random.randint(0, len(child_schedule) - 1)
            pos2 = random.randint(0, len(child_schedule) - 1)
            
            # 确保选择了两个不同的位置
            while child_schedule[pos1] == child_schedule[pos2]:
                pos2 = random.randint(0, len(child_schedule) - 1)
            
            # 创建新的调度序列，交换两个元素
            new_schedule = child_schedule.copy()
    ...
    def local_search(self, child_machine: List[int], child_agv_w: List[int],
        ...
        随机选择不同的局部搜索方法，包括机器分配、AGV分配和调度序列的局部搜索。
        ...
        """
        # 生成随机数，决定使用哪种局部搜索方法
        beta1 = random.random()  # 机器分配局部搜索方法选择
        beta2 = random.random()  # AGV分配局部搜索方法选择
        beta3 = random.random()  # 调度序列局部搜索方法选择
        ...
        # 根据beta1选择机器局部搜索方法
        if beta1 < 0.5:
            machine_ls_method = "machine_local_search1"
            new_machine, new_makespan = self.machine_local_search1(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        else:
            machine_ls_method = "machine_local_search2"
            new_machine, new_makespan = self.machine_local_search2(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        ...
        if new_makespan < current_makespan:
            ...
            logger.debug(f"{machine_ls_method}成功！完工时间从 {current_makespan:.2f} 降低到 {new_makespan:.2f}，改进: {improvement:.2f} ({improvement_percentage:.2f}%)")
            return new_machine, child_agv_w, child_agv_f, child_schedule, new_makespan, f"{machine_ls_method}_Success：{improvement:.2f} ({improvement_percentage:.2f}%)"
        ...
        # 根据beta2选择AGV局部搜索方法
        if beta2 < 1/3:
            agv_ls_method = "agv_local_search1"
            new_agv_w, new_makespan = self.agv_local_search1(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        elif beta2 < 2/3:
            agv_ls_method = "agv_local_search2"
            new_agv_w, new_makespan = self.agv_local_search2(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        else:
            agv_ls_method = "agv_local_search3"
            new_agv_w, new_makespan = self.agv_local_search3(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        ...
        if new_makespan < current_makespan:
            improvement = current_makespan - new_makespan
            improvement_percentage = (improvement / current_makespan) * 100
            logger.debug(f"{agv_ls_method}成功！完工时间从 {current_makespan:.2f} 降低到 {new_makespan:.2f}，改进: {improvement:.2f} ({improvement_percentage:.2f}%)")
            return child_machine, new_agv_w, child_agv_f, child_schedule, new_makespan, f"{agv_ls_method}_Success：{improvement:.2f} ({improvement_percentage:.2f}%)"
        
        # 3. 调度序列局部搜索
        logger.debug(f"执行调度序列局部搜索，当前完工时间: {current_makespan:.2f}")
        
        # 根据beta3选择调度序列局部搜索方法
        if beta3 < 0.5:
            schedule_ls_method = "schedule_local_search1"
            new_schedule, new_makespan = self.schedule_local_search1(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        else:
            schedule_ls_method = "schedule_local_search2"
            new_schedule, new_makespan = self.schedule_local_search2(
                child_machine, child_agv_w, child_agv_f, child_schedule, current_makespan
            )
        ...
        if new_makespan < current_makespan:
            improvement = current_makespan - new_makespan
            improvement_percentage = (improvement / current_makespan) * 100
            logger.debug(f"{schedule_ls_method}成功！完工时间从 {current_makespan:.2f} 降低到 {new_makespan:.2f}，改进: {improvement:.2f} ({improvement_percentage:.2f}%)")
            return child_machine, child_agv_w, child_agv_f, new_schedule, new_makespan, f"{schedule_ls_method}_Success：{improvement:.2f} ({improvement_percentage:.2f}%)"
        
        # 如果所有局部搜索都没有改进，返回原始解
        logger.debug("所有局部搜索方法都未能改进当前解")
        ...
        for generation in range(self.max_iterations):
            ...
            for i in range(self.population_size):
                ...
                if makespan is not None:
                    ...
                    theta = random.random()
                    # 计算当前解与全局最优解的差距，并设置执行局部搜索的概率
                    ls_probability = min(math.exp(-(makespan - self.best_solution[1])), 0.05)

                    if theta < ls_probability:  
                        # 局部搜索
            ...
            best_id, best_makespan, best_machine, best_agv_w, best_agv_f, best_schedule = best_individual
            # 执行局部搜索
            original_makespan = best_makespan
            new_machine, new_agv_w, new_agv_f, new_schedule, new_makespan, improvement_info = self.local_search(
                best_machine, best_agv_w, best_agv_f, best_schedule, best_makespan
            )
            # 如果局部搜索有改进，则替换原个体
            if new_makespan < best_makespan:
                self.population[0] = (best_id, new_makespan, new_machine, new_agv_w, new_agv_f, new_schedule)
                # 如果改进后的解优于当前全局最优解，则更新全局最优解
                if new_makespan < self.best_solution[1]:
                    self.best_solution = self.population[0]
                    self.best_generation = generation + 1  # 更新最好代数
