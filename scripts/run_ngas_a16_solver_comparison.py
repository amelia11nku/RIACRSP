#!/usr/bin/env python3
"""Run the frozen, resumable 54-run NGAS A1.6 solver comparison."""
...
def build_tasks(config: dict) -> list[dict]:
    ...
    if len(tasks) != 54 or len({(row['instance_id'], row['seed']) for row in tasks}) != 54:
        raise RuntimeError('A1.6 requires exactly 54 unique instance-seed runs')
    return tasks
...
def load_boundary(require_clean: bool = True) -> tuple[dict, dict, str]:
    ...
    if protocol.get('schema') != 'ngas-a16-formal-protocol-v1' \
            or protocol.get('status') != 'FROZEN_BEFORE_FORMAL_RESULTS':
        raise RuntimeError('A1.6 formal protocol is not frozen')
    if digest(CONFIG) != protocol['config_sha256']:
        raise RuntimeError('A1.6 config changed after freeze')
    for relative, expected in protocol['source_hashes'].items():
        if digest(ROOT / relative) != expected:
            raise RuntimeError(f'A1.6 frozen source changed: {relative}')
    for relative, expected in protocol['artifact_hashes'].items():
        if digest(ROOT / relative) != expected:
            raise RuntimeError(f'A1.6 frozen artifact changed: {relative}')
    ...
    if digest(preformal_path) != protocol['preformal_audit_sha256'] \
        ...
        raise RuntimeError('A1.6 preformal audit is absent, changed, or failed')
    ...
    if live_audit['status'] != 'PASS' \
        ...
        raise RuntimeError('A1.6 frozen comparison inputs changed')
    if forbidden_holdout_access_exists():
        raise RuntimeError('R13/R14 access is prohibited during A1.6')
    if require_clean and subprocess.check_output(
            ['git', 'status', '--porcelain'], cwd=ROOT, text=True).strip():
        raise RuntimeError('A1.6 formal execution requires a clean committed worktree')
...
def validate_existing_raw(path: Path, task: dict, protocol_sha256: str,
    ...
    validate_raw_contract(payload, task, protocol_sha256)
    if payload.get('config_sha256') != digest(CONFIG) \
        ...
        raise RuntimeError(f'Existing A1.6 raw payload failed metadata validation: {path}')
    instance_path = ROOT / config['scope']['instance_root'] / task['instance_relative_path']
    if digest(instance_path) != task['instance_sha256']:
        raise RuntimeError(f'A1.6 instance changed: {instance_path}')
    ...
    if (not replay_record['feasible']
        ...
        raise RuntimeError(f'Existing A1.6 raw payload failed replay/integrity: {path}')
    return payload
...
def write_progress(tasks: list[dict], completed: set[tuple[str, int]],
    ...
    atomic_json(PROGRESS, {
        ...
        'resume_command': f'{sys.executable} -u scripts/run_ngas_a16_solver_comparison.py --device cuda:0',
...
def execute(task: dict, protocol_sha256: str, config: dict, critic,
            runtime: ProductionRefreshRuntime, search_config) -> dict:
    instance_path = ROOT / config['scope']['instance_root'] / task['instance_relative_path']
    if digest(instance_path) != task['instance_sha256']:
        raise RuntimeError(f"A1.6 instance hash changed: {task['instance_id']}")
    instance = load_instance(instance_path)
    ...
    if not replay.feasible or not replay_audit['feasible'] \
        ...
        raise RuntimeError(f"A1.6 final replay failed: {task['instance_id']} seed {task['seed']}")
    ...
    if (not trace or trace[-1]['current_best_makespan'] != result.best.makespan
        ...
        raise RuntimeError('A1.6 incumbent trace failed monotonicity validation')
    ...
    if not all_finite(payload):
        raise RuntimeError('A1.6 raw payload contains non-finite values')
    validate_raw_contract(payload, task, protocol_sha256)
    return payload
...
def main() -> None:
    ...
    if not args.device.startswith('cuda') or not torch.cuda.is_available():
        raise RuntimeError('Formal A1.6 requires the qualified CUDA environment')
    ...
    existing_manifest = load_json(existing_manifest_path) if existing_manifest_path.exists() else {
        'schema': 'ngas-a16-raw-manifest-v1', 'files': {}
    }
    for relative, expected in existing_manifest.get('files', {}).items():
        if digest(ROOT / relative) != expected:
            raise RuntimeError(f'A1.6 raw changed after manifesting: {relative}')
    completed = set()
    for task in tasks:
        path = raw_path(task)
        if path.exists():
    ...
    write_progress(tasks, completed, pending[0] if pending else None,
                   process_started, protocol_sha256)
    print(json.dumps({
        'event': 'a16_formal_start', 'pending_runs': len(pending),
        'completed_runs': len(completed),
    ...
    for index, task in enumerate(pending, 1):
        print(json.dumps({
            'event': 'a16_run_start', 'pending_index': index,
            'pending_total': len(pending), 'instance_id': task['instance_id'],
            'seed': task['seed'], 'budget_seconds': task['budget_seconds'],
        }), flush=True)
        payload = execute(task, protocol_sha256, config, critic, runtime, search_config)
        path = raw_path(task)
        write_new_json(path, payload)
        validate_existing_raw(path, task, protocol_sha256, config)
        completed.add((task['instance_id'], task['seed']))
        manifest = {
            'schema': 'ngas-a16-raw-manifest-v1',
            'protocol_sha256': protocol_sha256,
            'completed_runs': len(completed),
            'files': {
        ...
        write_progress(tasks, completed, next_task, process_started, protocol_sha256)
        print(json.dumps({
            'event': 'a16_run_complete', 'completed_runs': len(completed),
    ...
    print(json.dumps({
        'event': 'a16_formal_raw_complete', 'completed_runs': len(completed),
        'protocol_sha256': protocol_sha256,
    }), flush=True)
