#!/usr/bin/env python3
"""One E3 stability amendment: compile dynamic graphs without CUDA Graph retention."""
import json
...
CONFIG = ROOT / 'configs/phase6k_runtime_v1_stability_amended.json'
...
def main():
    ...
    if settings['status'] != 'E3_STABILITY_AMENDMENT_FROZEN_BEFORE_EXECUTION':
        raise RuntimeError('invalid stability amendment status')
    failure = c.r.load_json(c.OUT / 'formal_latency/result.json')
    if failure['failure'] != 'CUDA_OUT_OF_MEMORY_DURING_VARIABLE_SHAPE_FORMAL_ASSESSMENT':
        raise RuntimeError('stability amendment has no matching trigger')
    state_ids = c.r.load_json(c.OUT / 'e1_equivalence/protocol.json')['state_ids']
    if len(state_ids) != 288:
        raise RuntimeError('the frozen R12 state list is incomplete')
    directory = c.freeze_stage('e3_stability', {
        'stability_config_sha256': c.digest(CONFIG),
        'failed_formal_sha256': c.digest(c.OUT / 'formal_latency/result.json'),
    ...
    if sorted(state_ids) != sorted(env['frames']):
        raise RuntimeError('active R12 state list changed')
    inductor_config.triton.cudagraph_skip_dynamic_graphs = True
    eager = VectorizedEnsemble(eager_reconstruction(env['e0']))
    gate = DeviceDecision(env['protocol']).cuda().eval()
    compiler = settings['compiler']
    model = torch.compile(eager, mode=compiler['mode'], fullgraph=compiler['fullgraph'],
    ...
    result = {'status': 'PASS' if passed else 'FAIL', 'variant': 'E3S',
    ...
    print(json.dumps({'status': result['status'], 'candidate_rows': result['candidate_rows'],
        ...
        'minimum_driver_free_bytes': result['minimum_driver_free_bytes'],
