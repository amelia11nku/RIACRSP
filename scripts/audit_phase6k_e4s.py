#!/usr/bin/env python3
"""Audit the single preregistered hoisted-Q/K/V E4S candidate."""
import json
...
CONFIG = ROOT / 'configs/phase6k_runtime_v1_e4s.json'


def main():
    settings = c.r.load_json(CONFIG)
    if settings['status'] != 'E4S_FROZEN_BEFORE_IMPLEMENTATION':
        raise RuntimeError('invalid E4S amendment status')
    for key in ('component_profile', 'e3s_result', 'e3s_selection'):
        path = ROOT / settings['selection_trigger'][key]
        if c.digest(path) != settings['selection_trigger'][key + '_sha256']:
            raise RuntimeError(f'E4S trigger changed: {key}')
    state_ids = c.r.load_json(c.OUT / 'e1_equivalence/protocol.json')['state_ids']
    directory = c.freeze_stage('e4s_equivalence', {
        'e4s_config_sha256': c.digest(CONFIG),
        'state_ids': state_ids,
    ...
    result = {'status': 'PASS' if passed else 'FAIL', 'variant': 'E4S',
