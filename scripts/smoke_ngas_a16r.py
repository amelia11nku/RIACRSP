#!/usr/bin/env python3
"""Run one explicitly excluded CUDA smoke for A1.6R instrumentation."""
...
OUT = ROOT / 'outputs/ngas_a1/solver_comparison_a16r_v1/smoke/smoke.json'
...
def main() -> None:
    ...
    if OUT.exists():
        ...
        if (payload.get('schema') != 'ngas-a16r-excluded-smoke-v1'
                or payload.get('config_sha256') != digest(CONFIG)
                or payload.get('excluded_from_formal') is not True
                or payload.get('status') != 'PASS'):
            raise RuntimeError('Existing A1.6R smoke artifact is invalid and will not be overwritten')
        print(json.dumps({'status': 'VALID_EXISTING_SMOKE', 'path': str(OUT.relative_to(ROOT))}))
        return
    if not torch.cuda.is_available():
        raise RuntimeError('A1.6R smoke requires CUDA')
    frozen = audit_frozen_inputs(ROOT, config)
    if frozen['status'] != 'PASS':
        raise RuntimeError('A1.6R frozen input audit failed before smoke')
    ...
    checks = {
        ...
        'a16r_diagnostics_present': (
            result.diagnostics['a16r_instrumentation']['enabled']
            and all('a16r_observation' in row
                    for row in result.diagnostics['iterations'])),
    }
    payload = {
        'schema': 'ngas-a16r-excluded-smoke-v1',
