#!/usr/bin/env python3
"""Freeze A1.6R integrity, diagnostics, source, and decision boundaries."""
...
def main() -> None:
    if PROTOCOL.exists():
        raise RuntimeError('A1.6R protocol already exists and is immutable')
    if (subprocess.run(['git', 'diff', '--quiet'], cwd=ROOT).returncode != 0
            or subprocess.run(['git', 'diff', '--cached', '--quiet'], cwd=ROOT).returncode != 0):
        raise RuntimeError('Freeze A1.6R only after committing the tracked implementation')
    ...
    if preformal.get('status') != 'PASS' or frozen.get('status') != 'PASS' \
        ...
        raise RuntimeError('A1.6R preformal or frozen-input audit is not valid')
    current_commit = subprocess.check_output(
        ['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip()
    if preformal.get('implementation_commit') != current_commit:
        raise RuntimeError('Tracked implementation changed after A1.6R preformal audit')
    formal_raw = list((OUT / 'raw').glob('*/*.json')) if (OUT / 'raw').exists() else []
    if formal_raw:
        raise RuntimeError('Formal A1.6R raw exists before protocol freeze')
    source_paths = [
        'scripts/run_ngas_a16r_solver_comparison.py',
        'scripts/launch_ngas_a16r_solver_comparison.py',
        'scripts/recover_ngas_a16r_stale_lock.py',
        'scripts/smoke_ngas_a16r.py',
        'scripts/audit_ngas_a16r_capacity.py',
        'scripts/audit_ngas_a16r_equivalence.py',
        'scripts/audit_ngas_a16r_preformal.py',
        'scripts/freeze_ngas_a16r_integrity_diagnostic.py',
        'scripts/freeze_ngas_a16r_offline_audit.py',
        'scripts/run_ngas_a16r_offline_audit.py',
        'scripts/finalize_ngas_a16r_integrity_diagnostic.py',
        'rcias_ngas/evaluation/a16.py',
        'rcias_ngas/evaluation/a16r_diagnostics.py',
        'rcias_ngas/evaluation/a16_io.py',
        'rcias_ngas/evaluation/a16_integrity.py',
        'rcias_ngas/execution/__init__.py',
        'rcias_ngas/execution/session_lock.py',
        ...
        'tests/ngas/test_a16r_integrity_diagnostics.py',
    ]
    hashes = {path: digest(ROOT / path) for path in source_paths}
    atomic_json(SOURCE_HASHES, {
        'schema': 'ngas-a16r-source-hashes-v1',
        'implementation_commit': current_commit,
        'files': hashes,
    })
    artifact_paths = [
        'configs/ngas_a16r_integrity_diagnostic_v1.json',
        ...
        'outputs/ngas_a1/solver_comparison_v1/audit/completion_audit.json',
        'outputs/ngas_a1/solver_comparison_v1/final_decision.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/source_hashes.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/comparator_audit.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/preformal_audit.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/regression.log',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/capacity_audit.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/preregistration/behavior_invariance.json',
        'outputs/ngas_a1/solver_comparison_a16r_v1/smoke/smoke.json',
    ]
    ...
    protocol = {
        'schema': 'ngas-a16r-formal-protocol-v1',
        'status': 'FROZEN_BEFORE_FORMAL_RESULTS',
        'frozen_at_utc': datetime.now(timezone.utc).isoformat(),
        'starting_a16_invalid_completion_commit': config['starting_commit'],
        'implementation_commit': current_commit,
        ...
        'formal_owner_id': config['integrity']['formal_owner_id'],
        ...
        'formal_command': [
            ...
            'scripts/run_ngas_a16r_solver_comparison.py', '--device', 'cuda:0'],
        'finalizer_command': [
